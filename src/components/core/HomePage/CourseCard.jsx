...
const CourseCard = (props) => {
    ...
    return (
        <div  className={`flex flex-col items-center justify-between mt-10 h-[250px] w-[341px] cursor-pointer ${isActive ? "bg-white text-[#585D69] shadow-[5px_5px_0_0] shadow-yellow-25" : "bg-[#161D29] text-[#6E727F] hover:bg-richblack-700"}`}
        onClick={() => setCurrentCard(cardData.heading)}>
          <div className="flex flex-col gap-[12px]">
            <h2 className={`mt-3 font-500 font-inter text-lg ${isActive ? "text-black" :"text-white"}`}>{cardData.heading}</h2>
            <p className={`${isActive ? "text-[#6E727F]" :"text-[#6E727F] hover:text-richblack-50"} w-[300px] items-start`}>{cardData.description}</p>
          </div>
